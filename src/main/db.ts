...
export function initializeAllLocationsCache(callback?: (err: Error | null, count?: number) => void): void {
  ...
  db_locations.all<ArcPoint>('SELECT * FROM locations ORDER BY time ASC', (err, rows) => {
    ...
    console.log(`[Cache] Successfully cached ${count} locations into ${dailyChunksCache.size} daily chunks.`);
...
export function getAllLocationsInRange(
  ...
  if (!dailyChunksCache || !allLocationsMasterCache) {
    const cacheError = new Error('[Cache] Location cache not initialized. Call initializeAllLocationsCache on startup.');
  ...
  if (processedLocations.length > MAX_POINTS_TO_RENDER) {
    console.log(`[Cache] Subsampling from ${processedLocations.length} to ${MAX_POINTS_TO_RENDER} points for range.`);
